...
public:
	...
	void setGridSize(uint8_t gsize){
		grid_w/=(gsize+1);
		grid_h/=(gsize+1);
		grid_pix=gsize;
	}

	uint8_t get_grid_w() const{ return grid_w;}
	uint8_t get_grid_h() const{ return grid_h;}

	void fillGridPixel(uint8_t x, uint8_t y, uint8_t color){
	    uint8_t x_grid = x*(grid_pix+1);
	    uint8_t y_grid = y*(grid_pix+1);
	    
	    for(int i=x_grid; i<x_grid+grid_pix; i++){
    	    for(int j=y_grid; j<y_grid+grid_pix; j++){
        	    drawPixel(i,j,color);
        	}
   		}
	}

	void fillGridRect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t color){
    for(int i=x; i<x+w; i++){
        for(int j=y; j<y+h; j++){
            fillGridPixel(i,j,color);
        }
    }
}
